...
@php
    $scooters = App\Models\Scooter::all();
@endphp
@section('content')
    <div class="statistics_wrapper" id="statistics_wrapper">
        <div class="switchs">
            <button :class="showScooters ? 'active' : ''"
                @click="showScooters = true;showTechnicians = false; showAccountant = false; showSellers = false">
                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-scooter" width="70"
                    height="70" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" fill="none"
            ...
            <button :class="showSellers ? 'active' : ''"
                @click="showScooters = false;showTechnicians = false; showAccountant = false; showSellers = true">
                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-building-store" width="70"
                    height="70" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" fill="none"
                    stroke-linecap="round" stroke-linejoin="round">
                    {{-- <path stroke="none" d="M0 0h24v24H0z" fill="none"/> --}}
                    <path d="M3 21l18 0" />
                ...
                <tbody>
                    @foreach ($scooters as $scooter)
                        <tr style="width: 100%">
                            ...
                            <td>{{ number_format($scooter->total_trips_number) }}</td>
                            <td>{{ number_format($scooter->total_duration_cost) }}</td>
                        </tr>
                    @endforeach
                    @if ($scooters->count() == 0)
                ...
                <div class="flex-center">
                    <button class="add-btn primary" @click="showFilterSellers = true">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-calendar-month"
                            width="40" height="40" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50"
                            fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                        ...
                        <span class="period" v-if="isFillterApplied && from && to">
                            ...
                            From <span>@{{ new Date(this.from).toLocaleDateString('en-US', {
    day: '2-digit',
    month: 'short',
    year: '2-digit'
}) }}</span> TO <span>@{{ new Date(this.to).toLocaleDateString('en-US', {
    day: '2-digit',
    month: 'short',
    year: '2-digit'
}) }}</span>
                    ...
                    <button class="add-btn" @click="seller_title = 'Seller';showAddSeller = true">Add <i
                            class="bx bx-plus"></i></button>
                    <div class="form-group search">
                        <input type="text" name="search" id="search" placeholder="Search Seller" class="input"
                            v-model="search" @input="getSellersbySearch()">
                        <i class='bx bx-search'></i>
                    </div>
                </div>
            </div>
                    ...
                    <tr v-if="sellers && sellers.length > 0" v-for="seller in sellers" :key="seller.id">
                        ...
                        <td>
                            <div class="btns flex-center">
                                <a class="button primary" :href="`/admin/seller/${seller.id}`"><i
                                        class='bx bx-show'></i></a>
                                <button class="button success" @click="handleEditSeller(seller)"><i
                                        class='bx bx-edit'></i></button>
                                <button class="button danger" @click="deleteSeller(seller.id, seller.name)"><i
                                        class='bx bx-trash'></i></button>
                            </div>
                        </td>
                    </tr>
                    <tr v-if="!sellers || sellers.length == 0"
                        style="font-size: 20px; font-weight: 700; text-align: center">
                        <td colspan="6">
                            <h2>There is no Sellsers!</h2>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="hide-content" @click="showAddSeller = false"
            v-if="showAddSeller | showEditSeller | showFilterSellers"></div>
        <div class="pop-up show_request_details_wrapper card" v-if="showAddSeller">
            ...
            <br>
            <div class="form-group">
                <input type="text" name="email" id="email" class="form-control input" v-model="email"
                    placeholder="Email">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="phone" id="phone" class="form-control input" v-model="phone"
                    placeholder="Phone">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="address" id="address" class="form-control input" v-model="address"
                    placeholder="address">
            </div>
            <br>
            <div class="form-group">
                <input type="password" name="password" id="password" class="form-control input" v-model="password"
                    placeholder="Password">
            </div>
            <br>
            <div class="form-group">
                <input type="password" name="password_confirmation" id="password_confirmation"
                    class="form-control input" v-model="password_confirmation" placeholder="password_confirmation">
            </div>
            <br>
            <div class="btns flex-center">
        ...
        <div class="pop-up show_request_details_wrapper card" v-if="showEditSeller && seller_data">
            ...
            <br>
            <div class="form-group">
                <input type="text" name="name" id="name" class="form-control input" v-model="to_edit_name"
                    placeholder="Name">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="email" id="email" class="form-control input" v-model="to_edit_email"
                    placeholder="Email">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="phone" id="phone" class="form-control input" v-model="to_edit_phone"
                    placeholder="Phone">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="address" id="address" class="form-control input"
                    v-model="to_edit_address" placeholder="address">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="password" id="password" class="form-control input"
                    v-model="to_edit_password" placeholder="New Password (optional)">
            </div>
            <br>
            <div class="form-group">
                <input type="text" name="password" id="password" class="form-control input"
                    v-model="to_edit_password_confirmation" placeholder="New Password confirmation">
            </div>
            <br>
            <div class="btns flex-center">
        ...
        <div class="pop-up show_request_details_wrapper card" v-if="showFilterSellers">
            ...
            <div class="form-group">
                <label for="date_from"
                    style="font-size: 18px;margin-bottom: 10px;text-align: left;display: block;padding: 0 1rem;font-weight: 500;">From</label>
                <input type="date" name="date_from" id="date_from" class="form-control input" v-model="from"
                    placeholder="Date From">
            </div>
            <br>
            <div class="form-group">
                <label for="date_from"
                    style="font-size: 18px;margin-bottom: 10px;text-align: left;display: block;padding: 0 1rem;font-weight: 500;">To</label>
                <input type="date" name="date_to" id="date_to" class="form-control input" v-model="to"
                    placeholder="Date To">
            </div>
            <br>
            <div class="btns flex-center">
                ...
                async getSellers() {
                    ...
                    try {
                        const response = await axios.get(
                            `{{ route('get.sellers') }}?page=${this.sellers_current_page}`);
