...
@section('content')
    @php
        ...
        $total_soldPoints = 0;
        foreach ($sellers as $seller) {
            $total_soldPoints += $seller->sold_points;
        }

        $requested_earning = App\Models\Trip::all()->sum('duration') * 15;
        ...
        <section class="row-1">
            <div class="statistics">
                <div class="card" style="grid-column: span 2">
                    <h1>
                        Requested earning coins <br>
                        <span>
                            {{ number_format($requested_earning) }}
                        </span>
                    </h1>
                </div>
                <div class="card">
                    <h1>
                        Total trips <br>
                        <span>
                            {{ number_format(App\Models\Trip::all()->count()) }}
                        </span>
                    </h1>
                </div>
                <div class="card">
                    <h1>
                        <span>+</span> <br>
                        Sold points <br>
                        <span>{{ number_format($total_soldPoints) }}</span>
                    </h1>
                </div>
            ...
            <div class="card notification_wrapper">
                ...
                <textarea name="" id="msg" cols="30" rows="10" class="input" placeholder="Message"
                    ...
                    <tr>
                        ...
                        <th>
                            ...
                            <input type="text" name="title" id="title" placeholder="Title" class="input"
                                v-model="title" style="min-width: 170px">
                        </th>
                        <th>
                            <label for="code">Code</label>
                            <input type="text" name="code" id="code" placeholder="Code" class="input"
                                v-model="code" style="min-width: 170px">
                        </th>
                        <th>
                            <label for="gift">Gift</label>
                            <input type="number" name="gift" id="gift" placeholder="Gift Coins" class="input"
                                v-model="gift" style="min-width: 170px">
                        </th>
                        <th>
                            <label for="start">Start in</label>
                            <input type="date" name="start" id="start" placeholder="Start in" class="input"
                                v-model="start_in">
                        </th>
                        <th>
                            <label for="end">End in</label>
                            <input type="date" name="end" id="end" placeholder="End in" class="input"
                                v-model="end_in">
                        </th>
                        <th>
        ...
        <section class="row-2 table_wrapper" v-if="notifications && notifications.length > 0 || notifications_search_words">
            <div class="head">
                ...
                <div class="pagination" v-if="notifications_last_page > 1">
                    <button
                        @click="notifications_current_page > 1 ? notifications_current_page -= 1 : '';this.getNotifications(this.type, this.notifications_current_page)">
                    ...
                    <button
                        @click="notifications_current_page < notifications_last_page ? notifications_current_page += 1 : '';this.getNotifications(this.type, this.notifications_current_page)">
                ...
                <div class="form-group search">
                    <input type="text" name="search" id="search" placeholder="Search notifications"
                        class="input" v-model="notifications_search_words"
                        @input="this.notifications_current_page = 1;getNotifications(this.type, this.notifications_current_page)">
            ...
            <table class="normal_table" style="display: table">
                ...
                <tbody>
                    <tr v-if="notifications && notifications.length > 0" v-for="notification in notifications"
                        :key="notification.id">
    ...
    hour12: true
                        ...
                        <td>
                            <div class="btns flex-center" style="max-width: 200px">
                                <a href="" class="button secondary"
                                    @click.prevent="resendNotification(notification.id, notification.title)">Resend</a>
                                <a href="" class="button danger"
                                    @click.prevent="deleteNotification(notification.id, notification.title)">
                    ...
                    <tr v-if="!notifications || notifications.length == 0"
                        ...
                        <td colspan="5">
                            <h2>There is no notification yet!</h2>
                        </td>
                    </tr>
                </tbody>
            </table>
